...
exec(open("C:\\Users\\Jessica\\Desktop\\eyetracking_functions.py").read())

#Eyetracker type selection
E = Smi()

if E.__class__.__name__ == 'Smi':
	folder = "C:\\Users\\Jessica\\Desktop\\SMI\\"
elif E.__class__.__name__ == 'Eyelink':
	folder = "C:\\Users\\Jessica\\Desktop\\Eyelink\\"
else:
	raise Exception('Unknown eyetracker %s' % E.__class__.__name__)
	
list_to_exclude = E.list_to_exclude

subjects = range(0,0)

#Reading files functions	
def get_inter_filename(subjectnumber):
	return "results%i_inter.txt" % subjectnumber

def get_result_filename(subjectnumber):
	return "results%i.txt" % subjectnumber
...
#Returns barycentre of a set of lines	
def compute_barycentre(lines):
	...
	return E.barycentre_from_points(list_points)
...
def compute_velocity_lines(line1,line2):
	...
	unite = abs(t2 - t1)
	velocity_px = amplitude(get_point(line1),get_point(line2))/unite
...
def point_in_fixation(fixation, current_line):
	...
	return amplitude(barycentre,get_point(current_line)) <= min_dispersion and velocity <= min_velocity
...
def get_max_duration(unite,p1,p2):
	max_duration = abs(round(2.2*convert_px_to_degrees(amplitude(p1,p2)))) +21 #in ms, Carpenter, 1988
...
def add_saccade_between(unite, n_trial, fixation1, fixation2, result_file):
	...
	Trace.log('Max duration : %i' % max_duration, Precision.DETAIL)
...
def get_blink(trial,unite):
	...
	trial_filtered = [line for line in trial if E.is_line_recording(line)]
...
for subjectnumber in subjects:
	Trace.log('Subject %i' % subjectnumber, Precision.TITLE)
	if subjectnumber in list_to_exclude:
		Trace.log('Excluding subject', Precision.NORMAL)
		continue
	
	data = E.get_file_raw(folder, subjectnumber)
	...
	unite = E.get_unit_pre_transfo(data_1000[0])
	...
	for n_trial in range(len(data_1000)):
		...
		Trace.log('Blinks were retrieved. Here is the list \n ' + str(blinks), Precision.NORMAL)
		print_message = True
		trial_filtered = []
		for line in trial:
			if E.is_line_recording(line):
	...
	for trial in data_filtered:
		...
		for i_line in range (0,len(trial)):
			if E.is_line_recording(trial[i_line]):
				nb_lines += 1
				...
				#If the velocity is above the threshold, we add the previous points to the list and look for the next point without artifact.
					if velocity >= max_velocity:
						...
						for elt in range (post_added_line,i_line):
							if E.is_line_recording(trial[elt]):
		...
		for line in trial[post_added_line:]:
			if not artifact_got and E.is_line_recording(line):
				trial_filtered.append(line)
						
		data_filtered2.append(trial_filtered)
	
		if n_trial == 15 and subjectnumber == 5:
			Trace.log('Print post artefact processing: ' + str(trial_filtered), Precision.DETAIL)
	...
	results = open(folder + get_inter_filename(subjectnumber),"w")
	for n_trial in range(len(data_filtered2)):
		...
		for line in trial:
			...
			if len(total_blinks[str(n_trial)]) > 0 and t > int(total_blinks[str(n_trial)][0][1]):
				Trace.log('Blink encountered: ' + str(line), Precision.DETAIL)
				current_fixation['blink_after'] = True
				total_blinks[str(n_trial)].pop(0)
				Trace.log('Saving fixation', Precision.NORMAL)
				Trace.log('fixation : ' + str(current_fixation), Precision.DETAIL)
			...
			#We do not consider negative or close to zero lines, for they are often noise.
				...
				if float(line[3]) < 10.00 and float(line[4]) < 10.00:
					...
					Trace.log('Point %s not in fixation' % str(get_point(line)), Precision.NORMAL)
					Trace.log('Saving fixation', Precision.NORMAL)
					Trace.log('fixation : ' + str(current_fixation), Precision.DETAIL)
					if is_fixation_valid(current_fixation):
						fixations.append(current_fixation)
		...
		Trace.log("Exporting fixations", Precision.NORMAL)
		Trace.log(fixations_to_string(fixations), Precision.DETAIL)
		export_fixations(unite, n_trial, fixations, results)
	results.close()	
	
	#Writing messages at the right place
	results_inter = open(folder + get_inter_filename(subjectnumber),"r")
	results = open(folder + get_result_filename(subjectnumber),"w")
